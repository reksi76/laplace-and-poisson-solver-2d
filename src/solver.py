...
def laplace_gauss_seidel(phi, max_iter=10000, tol=1e-5, save_interval=50, fixed_mask=None):
    history = []
    for it in range(max_iter):
        old_phi = phi.copy()
...
def poisson_jacobi(phi, source_term, max_iter=10000, tol=1e-5, save_interval=50, fixed_mask=None):
    history = []
    new_phi = phi.copy()
    h2 = 1.0
    source_scale = 0.01
    ...
    for it in range(max_iter):
        old_phi = new_phi.copy()
        new_phi[1:-1, 1:-1] = 0.25 * (old_phi[2:, 1:-1] + 
                                      old_phi[:-2,1:-1] + 
                                      old_phi[1:-1, 2:] + 
                                      old_phi[1:-1, :-2] - 
                                      h2 * source_term[1:-1, 1:-1] * source_scale
                                      ) 
        if fixed_mask is not None:
            new_phi[fixed_mask] = phi[fixed_mask]
...
def poisson_gauss_seidel(phi, source_term, max_iter=10000, tol=1e-5, save_interval=50, fixed_mask=None):
    history = []
    new_phi = phi.copy()
    h2 = 1.0
    source_scale = 0.01
        ...
        for i in range(1, phi.shape[0] - 1):
            for j in range(1, phi.shape[1] - 1):
                ...
                phi[i,j] = 0.25 * (phi[i+1,j] + phi[i-1, j] +
                                   phi[i, j+1] + phi[i, j-1] -
                                   h2 * source_term[i, j] * source_scale
                                   )
